...
class DomesticCustomDesign extends StatefulWidget {
  DomesticCustomDesign({Key? key}) : super(key: key);
  static const String id = 'domestic_custom';

  @override
  _DomesticCustomDesignState createState() => _DomesticCustomDesignState();
}

class _DomesticCustomDesignState extends State<DomesticCustomDesign> {
  List<CartItem> cart = [];
  List<String> items = [];
  List<int> qty = [];
  List<double> rating = [];
  ...
  void getCurrentUser() {
    try {
      ...
      if (user != null) {
        ...
        showAlertDialog(context, "Error", "Error connecting to the servers.");
                    ...
                    onPress: () {
                      ...
                      if (items.length > 0 &&
                        ...
                        if (qty.contains(0) ||
                            rating.contains(0) ||
                            duration.contains(0)) {
                          showAlertDialog(context, "Input error",
                              "Cannot have a 0 in the items");
                        } else {
                          Navigator.pushNamed(
                            context,
                            DomesticCustomParameters.id,
                            arguments: PassItemDataDomesticCustom(
                                item: items,
                                qty: qty,
                                rating: rating,
                                duration: duration),
                        ...
                        showAlertDialog(context, "Input error", 'Add Items');
                    },
                  ),
                ],
              )
              ...
              InfoButton(
                ...
                onPress: () {
                  showAlertDialog(
                      context,
                      "Power",
                      'The rated power. Usually found behind or '
                          'underneath the electrical appliance. The largest value with a \"W\".');
                },
              )
            ],
          ),
              ...
              InfoButton(
                ...
                onPress: () {
                  showAlertDialog(context, "Time",
                      'The amount of time the device runs per day.');
                },
              ),
            ],
          ),
...
class PassItemDataDomesticCustom {
  PassItemDataDomesticCustom(
      {required this.item,
      required this.qty,
      required this.rating,
      required this.duration});
